  ...
  <div class="space-y-6">
    ...
    <div class="ytp-top-row">
      ...
      <div
        ...
        ref="formSection"
        class="ytp-top-main page-form-wrap scroll-mt-24"
      >
        <LazyNewDownload
          :item="item_form"
    ...
    <div
      ...
      class="ytp-panel flex flex-col items-center justify-center gap-4 border-dashed bg-muted/10 px-6 py-10 text-center"
    >
      <img
        :src="hero.image"
        :alt="hero.alt"
        data-hero
        class="h-[clamp(5.5rem,16vh,10rem)] w-auto select-none"
        draggable="false"
      />

      <div class="space-y-1">
        <p class="font-serif text-lg text-highlighted">{{ t('queue.empty') }}</p>
        <p class="text-sm text-toned">{{ t('queue.emptyHint') }}</p>
      </div>
    </div>
      ...
      <div class="w-full min-w-0 max-w-full space-y-4">
        ...
        <div v-if="showPendingCard" class="ytp-card flex items-center gap-3 p-3 sm:gap-4 sm:p-4">
          ...
          <div class="min-w-0 flex-1 space-y-2">
            <span class="block h-3 w-1/3 animate-pulse rounded bg-elevated" />
            <span class="block h-2 w-2/3 animate-pulse rounded bg-elevated/70" />
            <span class="block h-1.5 w-full overflow-hidden rounded-full bg-elevated">
              <span class="ytp-pending-bar" />
            </span>
          </div>

          <span class="shrink-0 text-xs text-toned">{{ t('queue.preparing') }}</span>
        </div>
